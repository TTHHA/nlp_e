...
class ExplainNLP(pl.LightningModule):
    ...
    def __init__(
        ...
        self.train_acc = torchmetrics.classification.Accuracy(task="multiclass", num_classes=3)
        self.valid_acc = torchmetrics.classification.Accuracy(task="multiclass", num_classes=3)
        self.output = []
        self.check_data = []
    ...
    def configure_optimizers(self):
        ...
        t_total = len(self.train_dataloader()) // getattr(self.args, 'accumulate_grad_batches', 1) * 20
    ...
    def on_validation_epoch_end(self): # thêm outs
        # log epoch metric
...
def train(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(args.save_path, '{epoch}-{valid_loss:.4f}-{valid_acc_end:.4f}'),
